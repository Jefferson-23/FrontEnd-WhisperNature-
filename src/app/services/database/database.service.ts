...
const URL_MESSAGES = "http://apitest.madcat.one:8080/messages/"
const URL_USER_LIST = "http://apitest.madcat.one:8080/users/";
const URL_POST_CHANNEL ="http://apitest.madcat.one:8080/channels/";
...
export class DatabaseService {
  ...
  getChat_One(id: number): Observable<Message[]> {
    const URL_MSG_BY_ID = `http://apitest.madcat.one:8080/messages/${id}`;
  ...
  getUser_One(id: number): Observable<User[]> {
    const URL_ONE_USER = `http://apitest.madcat.one:8080/users/search/${id}`;
  ...
  getQRCode_One(id: number): Observable<User_Channel[]> {
    const URL_GET_CHANNEL = `http://apitest.madcat.one:8080/channels/search/${id}`;
